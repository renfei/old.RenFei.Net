...
import net.renfei.dao.entity.CacheDO;
import net.renfei.dao.entity.CacheDOExample;
import net.renfei.dao.persistences.CacheDOMapper;
...
public class CacheService {
    ...
    @Autowired
    private CacheDOMapper cacheDOMapper;
    ...
    public String getRandomKey() {
        String key = "";
        int length = 4;//默认生成4位数的Key
        for (int i = 0; ; i++) {
            key = stringUtil.getRandomString(length);
            if (get(key) == null) {
                return key;
            } else if (i > 10) {
        ...
        if (key != null && key.length() > 0) {
            ...
            if (cacheDO != null) {
                ...
                if (value != null && value.length() > 0 && expires != null) {
                    ...
                    //立即过期
                    cacheDO.setExpires(new Date(new Date().getTime() - 1));
                }
